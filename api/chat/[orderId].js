...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const msg = { id:"msg-"+Date.now().toString(36)+"-"+Math.random().toString(36).slice(2,7), senderId:isAdmin?"admin":user.id, senderName:isAdmin?"Tang Hospitality Service":user.name, senderRole:isAdmin?"admin":"customer", text:text.trim(), timestamp:new Date().toISOString() };
