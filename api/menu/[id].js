...
export default async function handler(req, res) {
  ...
  if (req.method === "PUT") {
    ...
    const qty = quantity !== undefined && quantity !== "" ? Number(quantity) : cur.quantity ?? 0;
    products[idx] = { ...cur, ...(name!==undefined&&{name}), ...(category!==undefined&&{category}), ...(subcategory!==undefined&&{subcategory}), ...(description!==undefined&&{description}), ...(priceRWF!==undefined&&{priceRWF:Number(priceRWF)}), ...(priceUSD!==undefined&&{priceUSD:Number(priceUSD)}), ...(image!==undefined&&{image}), quantity:qty, availability:qty>0 };
